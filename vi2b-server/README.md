...
При первом подключении клиента сервер автоматически регистрирует его в базе данных в таблице `users`. Для начала необходимо подключиться к серверу. После подключения клиент получает список всех доступных комнат этого сервера. Далее пользователь может подключиться к комнате и начать общаться с другими участниками.
